...
class TestBucketLogdir:
    """`nebo serve --logdir hf://...` — validation and env plumbing.

    These stop before uvicorn starts: cmd_serve is patched so we observe the
    environment it prepares rather than booting a daemon.
    """

    @staticmethod
    def _serve(monkeypatch, argv, env=None):
        """Run `nebo serve ...`, capturing the env it would hand to uvicorn.

        `cmd_serve` configures the daemon by writing NEBO_* into os.environ,
        so the whole environment is snapshotted and restored — monkeypatch
        alone would not undo writes to variables that were unset going in,
        and a leaked NEBO_LOGDIR/NEBO_CACHE_PATH reshapes every later test.
        """
        import io
        import os
        from contextlib import redirect_stderr, redirect_stdout
        ...
        try:
            for key in (
                "NEBO_LOGDIR", "NEBO_REMOTE", "NEBO_REMOTE_EPHEMERAL",
                "NEBO_NO_LOCAL", "NEBO_POLL_INTERVAL", "HF_TOKEN",
                "NEBO_CACHE_PATH",
            ):
                os.environ.pop(key, None)
            os.environ.update(env or {})
            with patch("uvicorn.run", _fake_run), patch("nebo.cli._write_pid"), \
                    patch("nebo.cli._remove_pid"), \
                    redirect_stderr(err), redirect_stdout(out), \
                    patch("sys.argv", ["nebo"] + argv):
                try:
                    main()
                except SystemExit as e:
                    code = int(e.code or 0)
    ...
    def test_bucket_logdir_is_not_path_resolved(self, monkeypatch):
        """Path('hf://x').resolve() would yield $CWD/hf:/x."""
        code, env, err = self._serve(
            monkeypatch, ["serve", "--logdir", "hf://datasets/acme/runs"]
        )
        assert code == 0, err
        assert env["NEBO_LOGDIR"] == "hf://datasets/acme/runs"

    def test_bucket_logdir_is_canonicalized(self, monkeypatch):
        code, env, _ = self._serve(
            monkeypatch, ["serve", "--logdir", "hf://datasets/acme/runs/"]
        )
        assert code == 0
        assert env["NEBO_LOGDIR"] == "hf://datasets/acme/runs"

    def test_malformed_bucket_logdir_errors(self, monkeypatch):
        code, _, err = self._serve(
            monkeypatch, ["serve", "--logdir", "hf://datasets/acme"]
        )
        assert code == 2
        assert "owner" in err

    def test_hf_token_flag_reaches_the_environment(self, monkeypatch):
        code, env, _ = self._serve(monkeypatch, [
            "serve", "--logdir", "hf://datasets/acme/runs", "--hf-token", "hf_abc",
        ])
        assert code == 0
        assert env["HF_TOKEN"] == "hf_abc"

    def test_poll_interval_flag_reaches_the_environment(self, monkeypatch):
        code, env, _ = self._serve(monkeypatch, [
            "serve", "--logdir", "hf://datasets/acme/runs", "--poll-interval", "5",
        ])
        assert code == 0
        assert env["NEBO_POLL_INTERVAL"] == "5.0"

    def test_remote_rejects_a_bucket_uri(self, monkeypatch, tmp_path):
        """The writer appends to an open stream; object storage can't."""
        code, _, err = self._serve(monkeypatch, [
            "serve", "--logdir", str(tmp_path),
            "--remote", "hf://datasets/acme/runs",
        ])
        assert code == 2
        assert "must be a local directory" in err

    def test_bare_remote_needs_a_dir_with_a_bucket_logdir(self, monkeypatch):
        """There is no <logdir>/remote/ to default to."""
        code, _, err = self._serve(monkeypatch, [
            "serve", "--logdir", "hf://datasets/acme/runs", "--remote",
        ])
        assert code == 2
        assert "explicit directory" in err

    def test_bucket_logdir_with_an_explicit_local_remote_is_allowed(
        self, monkeypatch, tmp_path
    ):
        code, env, err = self._serve(monkeypatch, [
            "serve", "--logdir", "hf://datasets/acme/runs",
            "--remote", str(tmp_path / "r"),
        ])
        assert code == 0, err
        assert env["NEBO_LOGDIR"] == "hf://datasets/acme/runs"
        assert env["NEBO_REMOTE"] == str(tmp_path / "r")

    def test_cache_identity_is_stable_across_machines(self):
        """The cache key is a sha1 of the logdir; cwd must not leak in."""
        from nebo.server.cache import resolve_cache_path

        a = resolve_cache_path("hf://datasets/acme/runs")
        b = resolve_cache_path("hf://datasets/acme/runs/")
        c = resolve_cache_path("hf://datasets/acme/other")
        assert a == b
        assert a != c
